      ...
      <div class="container-fluid">
        <div class="row mt-3 centralizar">
          <div class="col-lg-6">
            <div class="card">
              <div class="card-body">
                <div class="card-title">Registrar novo conteúdo</div>
                <hr>
                <form action="../php/recebeConteudo.php" method="POST" enctype="multipart/form-data">
                  <div class="form-group">
                    <label for="input-content-name">Nome do Conteúdo</label>
                    <input type="text" class="form-control" id="input-content-name" name="nome_conteudo"
                      placeholder="Digite o nome do conteúdo" required>
                  </div>
                  <div class="form-group">
                    <label for="input-content-description">Descrição do Conteúdo</label>
                    <textarea class="form-control" id="input-content-description" name="descricao" rows="3"
                      placeholder="Digite uma breve descrição do conteúdo" required></textarea>
                  </div>
                  <div class="form-group">
                    <label for="curriculo">PDF do curso</label>
                    <i id="pdf-icon" class="fas fa-check-circle text-success" style="display: none; padding: 8px;"></i>
                    <label class="select-style" for="customFilePDF" >PDF do conteúdo</label>
                    <input type="file"  class="select-style"class="form-control-file" name="pdf" id="customFilePDF" accept="application/pdf" required onchange="handleFileUpload(event, 'pdf-icon')" />
                  </div>
                  
                  <div class="form-group">
                    <label for="curriculo">Capa do curso</label>
                    <i id="image-icon" class="fas fa-check-circle text-success" style="display: none; padding: 8px;"></i>
                    <label class="select-style" for="customFileImage">Imagem do conteúdo</label>
                    <input class="select-style" type="file" class="form-control-file" name="capa" id="customFileImage" accept="image/*" required onchange="handleImageUpload(event, 'image-icon')" />
                  </div>
                  <div class="form-group">
                    <label for="input-2">Setor</label>
                    <div class="custom-dropdown">
                      <select id="input-2" class="form-control" name="id_setor" required>
...
const handleFileUpload = (event, iconId) => {
  ...
  const handleImageUpload = (event, iconId) => {
    const file = event.target.files[0];
